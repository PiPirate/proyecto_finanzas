...
export function ExpenseGamePanel({ currentDay, playerBudget, onComplete, onClose }) {
  ...
  if (stage === 'choices') {
    ...
    const needsSpent = choicesMade.filter(c => c.category === 'needs').reduce((sum, c) => sum + c.cost, 0);
    const wantsSpent = choicesMade.filter(c => c.category === 'wants').reduce((sum, c) => sum + c.cost, 0);
    const totalSpentNow = needsSpent + wantsSpent;
    ...
    const savingsPercentage = totalEarned > 0 ? Math.round(((totalEarned - totalSpentNow) / totalEarned) * 100) : 0;
          ...
          <div className="progress-tracker">
            ...
            <div className="tracker-summary">
              <p>
                ...
                Gustos: ${wantsSpent} • 
                Ahorro: ${totalEarned - totalSpentNow}
              </p>
            </div>
          </div>
